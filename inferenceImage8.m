%% Setup
modelfile = "weights\best.onnx";
% yolo export model=best.pt format=onnx imgsz=416 simplify=True opset=17
labels = ["cardboard", "glass", "metal", "paper", "plastic", "trash"];
colorMap = lines(numel(labels));  % 6 distinct RGB colors

%% Import ONNX network as dlnetwork
net = importNetworkFromONNX(modelfile);
net = initialize(net, dlarray(zeros(416, 416, 3, 1), 'SSCB'));

%% Read and preprocess image
orig = imread("path_to_image");
img = imresize(orig, [416 416]);
img = single(img) / 255;
img = reshape(img, [416, 416, 3, 1]);
img = dlarray(img, 'SSCB');

%% Run inference
predict(net, img);
tic;
output = predict(net, img);
output = squeeze(output)';
inferenceTime = toc;
% disp("Output size: " + mat2str(size(output)));
fprintf("Inference time: %.0f ms\n", inferenceTime * 1000);

%% Postprocess
sigmoid = @(x) 1 ./ (1 + exp(-x));
boxes = output(:,1:4); obj = sigmoid(output(:,5)); classScores = sigmoid(output(:,6:end));
[score_cls, clsID] = max(classScores, [], 2);
scores = obj .* score_cls;

th = 0.3;
idx = scores > th;
boxes = boxes(idx,:); scores = scores(idx); clsID = clsID(idx);

if isempty(scores)
    disp("!! No detections !!");
    return;
end

% Get top class
[~, bestIdx] = max(scores);
boxes = boxes(bestIdx, :);
scores = scores(bestIdx);
clsID = clsID(bestIdx);
clsID = extractdata(clsID);

% Convert [xc yc w h] to [x y w h]
boxes(:,1:2) = boxes(:,1:2) - boxes(:,3:4)/2;
scaleBack = [size(orig,2)/416, size(orig,1)/416, size(orig,2)/416, size(orig,1)/416];
boxes = boxes .* scaleBack;

%% Visualization
x = boxes(1); y = boxes(2); w = boxes(3); h = boxes(4);
box = gather(extractdata(boxes(1,:)));
x = box(1); y = box(2); w = box(3); h = box(4);
labelH = 25;
labelY = max(y - labelH, 1);  % Clamp top label position

disp("Unique class indices (raw):");
disp(unique(clsID+1));

classStr = reshape(labels(clsID+1), [], 1);
scoreStr = scores(:) * 100; disp(scoreStr)
ann = compose("%s: %.1f%%", classStr, scoreStr);
color = colorMap(clsID+1, :) * 255;

out = insertShape(orig, "Rectangle", [x y w h], ...
    "Color", color, "LineWidth", 4); % Draw bounding box
out = insertShape(out, "FilledRectangle", [x, labelY, w, labelH], ...
    "Color", color, "Opacity", 1); % Draw filled label background above box
out = insertText(out, [x + 5, labelY - 1], ann, ...
    "BoxOpacity", 0, "TextColor", "white", "FontSize", 18); % Draw white label text on top

disp("Box being drawn:");
disp([x y w h]);

% imshow(out);